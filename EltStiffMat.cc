...
// Dirichlet boundary conditions, the right-hand side $f = 2\pi^2 \sin(\pi x) \sin(\pi y)$,
// exact solution of pressure $p = \sin(\pi x) \sin(\pi y) $, and velocity for calculating errors.
//
...
//	return_value = cos(M_PI*p[0])*cos(M_PI*p[1])*cos(M_PI*p[2]);
...
//  return_value = 3*M_PI*M_PI*cos(M_PI*p[0])*cos(M_PI*p[1])*cos(M_PI*p[2]);
  return_value = 2*M_PI*M_PI*cos(M_PI*p[0])*cos(M_PI*p[1]);
  return return_value;
}
...
//  return_value[0] = M_PI*sin(M_PI*p[0])*cos(M_PI*p[1])*cos(M_PI*p[2]);
//  return_value[1] = M_PI*cos(M_PI*p[0])*sin(M_PI*p[1])*cos(M_PI*p[2]);
//  return_value[2] = M_PI*cos(M_PI*p[0])*cos(M_PI*p[1])*sin(M_PI*p[2]);
  return_value[0] = M_PI*sin(M_PI*p[0])*cos(M_PI*p[1]);
  return_value[1] = M_PI*cos(M_PI*p[0])*sin(M_PI*p[1]);
  return return_value;
...
// <code>FESystem</code> defines finite element spaces in the interior and on edges of elements,
...
//     const Point<dim> top_right = ( Point<dim>(1,1));
...
//            {
//              if ((std::fabs(cell->face(face_number)->center()(1) - (2200)) < 1e-12)
//                  ||
//                  (std::fabs(cell->face(face_number)->center()(1) - (0)) < 1e-12)
//                  ||
//                  (std::fabs(cell->face(face_number)->center()(2) - (170)) < 1e-12)
//                  ||
...
// Add N.B.C term to right hand side.
//   for (unsigned int face_number=0; face_number<GeometryInfo<dim>::faces_per_cell; ++face_number)
...
//           }
 ...
 void WGDarcyEquation<dim>::process_solution ()
 {
	...
	Vector<double> interior_solution (interior_dof_handler.n_dofs());
	{
		...
		for (unsigned int index=0; cell!=endc; ++cell, ++interior_cell, ++index)
		{
			cell->get_dof_indices (local_dof_indices);
...
//  DataOut<dim> data_out;
//  data_out.attach_dof_handler (dof_handler);
//  data_out.add_data_vector (Coefficient<dim>, "coefficient");
//  data_out.build_patches (fe.degree);
//  std::ofstream output ("coefficient.vtk");
//  data_out.write_vtk (output);
